...
public class ProcessPanel extends JPanel {
    ...
    private JButton     btnAdd, btnRemove, btnClear, btnStart, btnPause, btnBlock, btnWake, btnTerminate;
    ...
    private JPanel buildTablePanel() {
        ...
        // Column setup
        String[] cols = {"PID", "Name", "Arrival", "Burst", "Priority", "Remaining", "State", "Java Thread", "Ticks"};
        ...
        table.setSelectionBackground(Theme.ACCENT_CYAN);
        table.setSelectionForeground(Theme.BG_DEEP);
        ...
        table.getColumnModel().getColumn(6).setCellRenderer(new DefaultTableCellRenderer() {
            @Override public Component getTableCellRendererComponent(JTable t, Object val,
                ...
                l.setFont(sel ? Theme.FONT_SUBHEAD : Theme.FONT_SMALL);
                l.setForeground(sel ? Theme.BG_DEEP : Theme.stateColour(state));
                l.setBackground(sel ? Theme.ACCENT_CYAN : (row%2==0 ? Theme.BG_PANEL : Theme.BG_ROW_ALT));
                l.setHorizontalAlignment(SwingConstants.CENTER);
                l.setBorder(sel
                        ? BorderFactory.createCompoundBorder(
                                BorderFactory.createMatteBorder(2, 0, 2, 0, Theme.BG_DEEP),
        ...
        table.setDefaultRenderer(Object.class, new DefaultTableCellRenderer() {
            @Override public Component getTableCellRendererComponent(JTable t, Object val,
                    boolean sel, boolean focus, int row, int col) {
                JLabel l = (JLabel) super.getTableCellRendererComponent(t,val,sel,focus,row,col);
                l.setFont(sel ? Theme.FONT_SUBHEAD : Theme.FONT_CODE);
        ...
        table.getColumnModel().getColumn(6).setCellRenderer(new DefaultTableCellRenderer() {
            @Override public Component getTableCellRendererComponent(JTable t, Object val,
                ...
                l.setFont(sel ? Theme.FONT_SUBHEAD : Theme.FONT_SMALL);
                l.setForeground(sel ? Theme.BG_DEEP : Theme.stateColour(state));
                l.setBackground(sel ? Theme.ACCENT_CYAN : Theme.alpha(Theme.stateColour(state), 25));
                l.setHorizontalAlignment(SwingConstants.CENTER);
                l.setBorder(sel
                        ? BorderFactory.createCompoundBorder(
                                BorderFactory.createLineBorder(Theme.BG_DEEP, 2),
                                new EmptyBorder(1, 4, 1, 4))
                        : new EmptyBorder(2, 4, 2, 4));
                l.setOpaque(true);
                return l;
        ...
        // Column widths
        int[] widths = {40, 100, 60, 60, 60, 70, 90, 110, 70};
        ...
        btnStart  = UIHelper.successButton("RUN");
        btnPause  = UIHelper.ghostButton("PAUSE");
        btnBlock  = UIHelper.ghostButton("BLOCK");
        btnWake   = UIHelper.primaryButton("WAKE");
        btnTerminate = UIHelper.dangerButton("TERM");
        btnRemove = UIHelper.dangerButton("✕  REMOVE");
        btnClear  = UIHelper.ghostButton("⊘  CLEAR ALL");
        btnStart.addActionListener(e -> startSelected());
        btnPause.addActionListener(e -> pauseSelected());
        btnBlock.addActionListener(e -> blockSelected());
        btnWake.addActionListener(e -> wakeSelected());
        btnTerminate.addActionListener(e -> terminateSelected());
    ...
    private void removeSelected() {
        ...
        if (ok) ctx.getIpcManager().unregisterProcess(pid);
        refresh();
    ...
    private void startSelected() {
        int pid = selectedPid();
        if (pid < 0) return;
        boolean ok = ctx.getProcessManager().startProcess(pid);
        refresh();
        flashStatus(ok ? "✓ P" + pid + " running on Java thread" : "⚠ Could not start P" + pid,
                ok ? Theme.ACCENT_GREEN : Theme.ACCENT_RED);
    }
    ...
    private void pauseSelected() {
        int pid = selectedPid();
        if (pid < 0) return;
        boolean ok = ctx.getProcessManager().pauseProcess(pid);
        refresh();
        flashStatus(ok ? "✓ P" + pid + " paused" : "⚠ Could not pause P" + pid,
                ok ? Theme.ACCENT_AMBER : Theme.ACCENT_RED);
    }

    private void wakeSelected() {
        int pid = selectedPid();
        if (pid < 0) return;
        boolean ok = ctx.getProcessManager().wakeProcess(pid);
        refresh();
        flashStatus(ok ? "✓ P" + pid + " moved to READY" : "⚠ Could not wake P" + pid,
                ok ? Theme.ACCENT_CYAN : Theme.ACCENT_RED);
    }

    private void blockSelected() {
        int pid = selectedPid();
        if (pid < 0) return;
        boolean ok = ctx.getProcessManager().blockProcess(pid);
        refresh();
        flashStatus(ok ? "âœ“ P" + pid + " moved to WAITING" : "âš  Could not block P" + pid,
                ok ? Theme.ACCENT_AMBER : Theme.ACCENT_RED);
    }
    ...
    private void terminateSelected() {
        int pid = selectedPid();
        if (pid < 0) return;
        boolean ok = ctx.getProcessManager().terminateProcess(pid);
        refresh();
        flashStatus(ok ? "✓ P" + pid + " terminated" : "⚠ Could not terminate P" + pid,
                ok ? Theme.ACCENT_RED : Theme.ACCENT_RED);
    }
    ...
    public void refresh() {
        Integer selectedPid = getCurrentlySelectedPid();
        ...
        for (Process p : procs) {
            tableModel.addRow(new Object[]{
                ...
                p.getRemainingTime(), p.getState().name(), p.getJavaThreadState(), p.getRuntimeTicks()
            });
            if (Objects.equals(selectedPid, p.getPid())) {
    ...
    private int selectedPid() {
        Integer pid = getCurrentlySelectedPid();
        if (pid == null) {
            flashStatus("Select a process first", Theme.ACCENT_AMBER);
            return -1;
        }
        return pid;
    }

    private Integer getCurrentlySelectedPid() {
        int row = table.getSelectedRow();
