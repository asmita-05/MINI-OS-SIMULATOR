...
/**
 * Main application window for the Mini OS Simulator GUI.
 *
 * Layout:
 *   ┌─────────────────────────────────────────────────────┐
...
public class MainWindow extends JFrame {
    ...
    private static final String[][] NAV_ITEMS = {
        ...
        {"IPC",        "âœ‰", "ipc",       "Exchange messages between processes"},
    };

    public MainWindow() {
        super("Mini OS Simulator");
    ...
    private JPanel buildHeader() {
        ...
        sub.setText(sub.getText() + "  Â·  IPC");

        sub.setText("  |  Process  |  Scheduling  |  Memory  |  Filesystem  |  IPC");
    ...
    private JButton buildNavButton(String label, String icon, String card, String tooltip) {
        String normalizedIcon = "IPC".equals(label) ? "â¬¡" : icon;
        normalizedIcon = "[]";
        final String navIcon = normalizedIcon;
        JButton btn = new JButton() {
            ...
            @Override protected void paintComponent(Graphics g) {
                ...
                g2.drawString(navIcon, 14 + (24-fm.stringWidth(navIcon))/2, getHeight()/2+5);
    ...
    private JPanel buildStatusBar() {
        ...
        bar.add(dot);
        bar.add(statusLabel("Mini OS Simulator"));
        bar.add(statusSep());
