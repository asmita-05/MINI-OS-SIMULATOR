...
public class MemoryCommandHandler implements CommandHandler {
    ...
    public String execute(CommandInput input, SimulationContext ctx) {
        ...
        return switch (sub) {
            ...
            case "auto"                -> handleAuto(input, ctx);
            default                    -> Formatter.red(
                    "Unknown sub-command '" + sub + "'. Use: alloc | free | map | init | info | auto\n");
        };
    }
    ...
    private String handleInfo(SimulationContext ctx) {
        ...
        return String.format(
                ...
                Formatter.cyan(ctx.getMemStrategy().getDisplayName() + " (only First-Fit in Phase 2)"),
                mm.getTotalMemory(), mm.getUsedMemory(), mm.getFreeMemory(),
                100.0 * mm.getUsedMemory() / mm.getTotalMemory());
    }

    private String handleAuto(CommandInput input, SimulationContext ctx) {
        // mem auto on|off [size-per-proc]
        if (input.getArgCount() < 2)
            return Formatter.red("Usage: mem auto <on|off> [bytes-per-process]\n");

        boolean on = input.getArg(1).equalsIgnoreCase("on");
        ctx.setAutoAllocMemory(on);
        if (input.getArgCount() >= 3) {
            int perProc = input.getIntArg(2, 64);
            ctx.setDefaultMemoryPerProc(perProc);
        }
        return Formatter.green("✓ Auto-memory allocation: " + (on ? "ON" : "OFF") +
                " (" + ctx.getDefaultMemoryPerProc() + " units/process).\n");
    }
